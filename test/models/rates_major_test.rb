...
class RatesMajorTest < ActiveSupport::TestCase
	...
	test "can't insert if major nonexistent" do
		rating = rates_majors(:albert_cs)
	...
	test "allows users to rate multiple degrees" do
		albert = users(:albert)
		econ = degrees(:econ)
		bw = degrees(:bw)
	...
	test "allows degrees to be rated by multiple users" do
		albert = users(:albert)
		alice = users(:alice)
		econ = degrees(:econ)
